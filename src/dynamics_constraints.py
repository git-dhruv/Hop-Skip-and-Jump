...
def AddCollocationConstraints(prog, planar_arm, context, N, x, u, lambda_c, timesteps):
  ...
  for i in range(N - 1):
    ...
    prog.AddConstraint(CollocationConstraintHelper_2, contact_lower_bound-eps, contact_upper_bound+eps,np.hstack([x[i], x[i+1], u[i], u[i+1], lambda_c[i], lambda_c[i+1]]) )
